...
def interface(input_tensor,avg_class,weights1,biases1,weight2,biases2):
    if avg_class==None:
...
def train(mnist):
    ...
    cross_enrtopy=tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=y,labels=tf.argmax(y_,1)
    )
    cross_enrtopy_mean=tf.reduce_mean(cross_enrtopy)
    regularizer=tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
    regularizer=regularizer(weights1)+regularizer(weights2)
    loss=cross_enrtopy_mean+regularizer
